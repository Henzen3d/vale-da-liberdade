...
3. **Cartões em Papel Branco (`#ffffff`)**: Todos os componentes de dados (`QuoteCard`, `DocumentCard`, `TimelineCard`, `DataChartCard`, `ComparisonCard`, `RecorteCard`, **`XPostCard` / `#xCard`**) são construídos em fundo branco puro/marfim notarial com molduras douradas (ou chrome do X no Modo 8) e tipografia em preto editorial (`#0a0e17` / `#0f172a`), garantindo máxima legibilidade.
...
7. **Canvas 1080p oficial:** `references/youtube/mockup-browser/mockup-browser.html`. O typo histórico `mockup-brower.html` permanece como **alias sincronizado** para processos legados. `MOCKUP_HTML` em `scripts/bm_video/constants.py` prioriza o nome correto.
8. **Modo 8 — Card do X (`#xCard` / `.bcard-x-post`):** operacional ponta a ponta. Identidade (avatar ou monograma SVG, nome, selo, @handle), corpo com links, mídia `#xMediaBox` (`name=large` → `/shots/x-media-{id}.jpg`), métricas + like `#f91880`. GSAP `transitionToX`: browser desce, card sobe (`y: 140→0`, `scale: 0.95→1`, elastic). `update()` roteia `kind: "x-post" | "x" | "tweet"`.
9. **Broadcast Studio V2:** `references/youtube/mockup-browser/demo-broadcast-studio.html` (iframe → `mockup-browser.html`). Switcher Modos 1–8. Abrir: `python scripts/abrir_demo_broadcast.py` (porta 8765).

Índice de telas (switcher):

| Modo | kind | Tela |
|------|------|------|
| 1 | `source` | Portal de notícias (navegador + ticker) |
| 2 | `quote` | Aspas editorial / Card Gold |
| 3 | `document` | Decisão judicial / document zoom |
| 4 | `timeline` | Linha do tempo |
| 5 | `chart` | Impacto econômico |
| 6 | `comparison` | Split A × B |
| 7 | `recorte` | Recorte impresso |
| 8 | `x-post` | Publicação do X |

Spec de composição (crop/overlay/L3): `docs/BM-VIDEO-LAYOUT.md`.
...
|------|---------|--------|------|
| 2026-09-12 | Canvas oficial `mockup-browser.html`; alias `mockup-brower.html`; Modo 8 (x-post) ponta a ponta; Studio V2 | `09f1ec2` | `mockup-browser.html`, `demo-broadcast-studio.html`, `bm_video/constants.py`, `tests/test_x_post_mockup_integration.py` |
| 2026-09-06 | Light Editorial Broadcast Standard oficializado como padrão para todos os vídeos/broadcast | — | `mockup-browser.html` (alias `mockup-brower.html`), `LIGHT_BROADCAST_DESIGN_SYSTEM.md`, `bm_mockup_video.py` |
...
*Última atualização: 2026-09-12. Edite livremente; se mudar comportamento
