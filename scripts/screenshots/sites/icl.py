#!/usr/bin/env python3
"""Scraper cirúrgico para o ICL Notícias (iclnoticias.com.br).

Arquitetura do site:
- CMS do Instituto Conhecimento Liberta focado em jornalismo independente e economia.
- O texto integral da matéria vem no HTML original dentro de ``article``, ``.entry-content`` ou ``.article-content``.
- Sem paywall, mas com banners de membresia e avisos LGPD.
- O cabeçalho com o logo do ICL Notícias é mantido no topo em posição estática.
"""
...
# ---------------------------------------------------------------------------
# JS cirúrgico de limpeza para ICL Notícias
# ---------------------------------------------------------------------------

_CLEANUP_ICL_JS = """() => {
  const removed = [];

  // 1. Remover banners de publicidade e chamadas de membresia
  const adSelectors = [
    '[class*="membership"]',
    '[class*="membro"]',
    '[id*="google_ads"]',
  ...
  // 3. Tornar o cabeçalho institucional (logo ICL) estático e visível
...
@register("iclnoticias.com.br")
class IclScraper(BaseScraper):
    """Handler cirúrgico para o ICL Notícias."""

    name = "icl"
    domains = ("iclnoticias.com.br",)

    def wait_for_content(self, page: Any) -> bool:
        """Espera o artigo carregar no DOM."""
        for _ in range(15):
            try:
                info = page.evaluate(_WAIT_ICL_CONTENT_JS)
                if info.get("found"):
                    return True
            except Exception:
                pass
    ...
    def cleanup(self, page: Any) -> dict:
        """Remoção cirúrgica de anúncios e popups do ICL Notícias."""
        try:
            result = page.evaluate(_CLEANUP_ICL_JS)
            return {"handler": "icl", **result}
        except Exception as exc:
            return {"handler": "icl", "error": str(exc)[:300]}
