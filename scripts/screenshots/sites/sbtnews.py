#!/usr/bin/env python3
"""Scraper cirúrgico para o SBT News (sbtnews.com.br).

Arquitetura do site:
- CMS do SBT focado em cobertura jornalística nacional e transmissões.
- O texto integral da matéria vem no HTML original dentro de ``article``, ``.article-content`` ou ``.news-text``.
- Sem paywall, mas com presença de:
  - Banners de publicidade no topo e laterais
  - Vídeos e embeds de reportagem
  - Banners LGPD
- O cabeçalho institucional com o logo do SBT News é mantido no topo de forma estática.
"""
...
# ---------------------------------------------------------------------------
# JS cirúrgico de limpeza para SBT News
# ---------------------------------------------------------------------------
...
_CLEANUP_SBT_JS = """() => {
  ...
  adSelectors.forEach(sel => {
    document.querySelectorAll(sel).forEach(el => {
      if (el.querySelector('article, h1, .article-content, .news-text')) return;
      el.remove();
      removed.push(sel);
    });
  });
  ...
  // 3. Tornar o cabeçalho institucional (logo SBT News) estático e visível
  document.querySelectorAll('header, .header, .site-header, .sbt-header, nav').forEach(el => {
  ...
  document.querySelectorAll('article *, .article-content *, .news-text *').forEach(el => {
...
@register("sbtnews.com.br", "sbtnews.sbt.com.br")
class SbtNewsScraper(BaseScraper):
    """Handler cirúrgico para o SBT News."""

    name = "sbtnews"
    domains = ("sbtnews.com.br", "sbtnews.sbt.com.br")

    def wait_for_content(self, page: Any) -> bool:
        """Espera o artigo carregar no DOM."""
        for _ in range(15):
            try:
                info = page.evaluate(_WAIT_SBT_CONTENT_JS)
                if info.get("found"):
                    return True
            except Exception:
                pass
    ...
    def cleanup(self, page: Any) -> dict:
        """Remoção cirúrgica de anúncios e banners do SBT News."""
        try:
            result = page.evaluate(_CLEANUP_SBT_JS)
            return {"handler": "sbtnews", **result}
        except Exception as exc:
            return {"handler": "sbtnews", "error": str(exc)[:300]}
