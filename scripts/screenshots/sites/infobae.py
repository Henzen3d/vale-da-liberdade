#!/usr/bin/env python3
"""Scraper cirúrgico para o Infobae (infobae.com).

Arquitetura do site:
- CMS Arc Publishing com foco em cobertura latino-americana e internacional.
- O texto integral da matéria vem no HTML original dentro de ``article`` ou ``.article-body``.
- Sem paywall rígido, com banners de publicidade e módulos Taboola.
- O cabeçalho institucional com o logo do Infobae é mantido no topo em posição estática.
"""
...
# ---------------------------------------------------------------------------
# JS cirúrgico de limpeza para Infobae
# ---------------------------------------------------------------------------
...
_CLEANUP_INFOBAE_JS = """() => {
  ...
  adSelectors.forEach(sel => {
    document.querySelectorAll(sel).forEach(el => {
      if (el.querySelector('article, h1, .article-body')) return;
      el.remove();
      removed.push(sel);
    });
  });
  ...
  // 3. Tornar o cabeçalho institucional (logo Infobae) estático e visível
  ...
  document.querySelectorAll('article *, .article-body *').forEach(el => {
...
@register("infobae.com")
class InfobaeScraper(BaseScraper):
    """Handler cirúrgico para o Infobae."""
    ...
    def wait_for_content(self, page: Any) -> bool:
        ...
        for _ in range(15):
            try:
                info = page.evaluate(_WAIT_INFOBAE_CONTENT_JS)
                if info.get("found"):
                    return True
            except Exception:
                pass
    ...
    def cleanup(self, page: Any) -> dict:
        """Remoção cirúrgica de anúncios do Infobae."""
        try:
            result = page.evaluate(_CLEANUP_INFOBAE_JS)
            return {"handler": "infobae", **result}
        except Exception as exc:
            return {"handler": "infobae", "error": str(exc)[:300]}
