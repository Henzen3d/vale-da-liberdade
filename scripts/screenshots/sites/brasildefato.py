#!/usr/bin/env python3
"""Scraper cirúrgico para o Brasil de Fato (brasildefato.com.br).

Arquitetura do site:
- CMS focado em notícias populares, movimentos sociais e política.
- O texto integral da matéria vem no HTML original dentro de ``article``, ``.content-text`` ou ``.entry-content``.
- Sem paywall, com anúncios leves e banners institucionais.
- O cabeçalho com o logo do Brasil de Fato é mantido no topo em posição estática.
"""
...
# ---------------------------------------------------------------------------
# JS cirúrgico de limpeza para Brasil de Fato
# ---------------------------------------------------------------------------

_CLEANUP_BDF_JS = """() => {
  const removed = [];

  // 1. Remover banners de publicidade e caixas promocionais
  const adSelectors = [
    '[id*="google_ads"]',
    '[id*="ad_"]',
    '[class*="publicidade"]',
  ...
  adSelectors.forEach(sel => {
    document.querySelectorAll(sel).forEach(el => {
      if (el.querySelector('article, h1, .content-text, .entry-content')) return;
      el.remove();
      removed.push(sel);
    });
  });
  ...
  // 3. Tornar o cabeçalho institucional (logo Brasil de Fato) estático e visível
  ...
  document.querySelectorAll('article *, .content-text *, .entry-content *').forEach(el => {
...
@register("brasildefato.com.br")
class BrasilDeFatoScraper(BaseScraper):
    """Handler cirúrgico para o Brasil de Fato."""

    name = "brasildefato"
    domains = ("brasildefato.com.br",)

    def wait_for_content(self, page: Any) -> bool:
        """Espera o artigo carregar no DOM."""
        for _ in range(15):
            try:
                info = page.evaluate(_WAIT_BDF_CONTENT_JS)
                if info.get("found"):
                    return True
            except Exception:
                pass
    ...
    def cleanup(self, page: Any) -> dict:
        """Remoção cirúrgica de anúncios do Brasil de Fato."""
        try:
            result = page.evaluate(_CLEANUP_BDF_JS)
            return {"handler": "brasildefato", **result}
        except Exception as exc:
            return {"handler": "brasildefato", "error": str(exc)[:300]}
