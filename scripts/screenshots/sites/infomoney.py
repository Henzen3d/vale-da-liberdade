#!/usr/bin/env python3
"""Scraper cirúrgico para o InfoMoney (infomoney.com.br).

Arquitetura do site:
- CMS WordPress VIP focado em mercado financeiro, economia e política.
- O texto integral da matéria vem no HTML original dentro de ``article``, ``.article-content`` ou ``.entry-content``.
- Sem paywall rígido, mas com:
  - Banners de publicidade pesados (corretoras, relatórios e fundos)
  - Modais de captura de leads/newsletter
  - Barras flutuantes de cotações e recomendações
- O cabeçalho com a marca/logo do InfoMoney é mantido no topo de forma estática.
"""
...
# ---------------------------------------------------------------------------
# JS cirúrgico de limpeza para InfoMoney
# ---------------------------------------------------------------------------

_CLEANUP_INFOMONEY_JS = """() => {
  const removed = [];
  ...
  adSelectors.forEach(sel => {
    document.querySelectorAll(sel).forEach(el => {
      if (el.querySelector('article, h1, .article-content, .entry-content, .post-content')) return;
      el.remove();
      removed.push(sel);
    });
  });

  // 2. Remover modais de captura, popups e avisos de cookies
  ...
  // 3. Tornar o cabeçalho institucional (logo InfoMoney) estático e visível
  ...
  document.querySelectorAll('article *, .article-content *, .entry-content *').forEach(el => {
...
@register("infomoney.com.br")
class InfoMoneyScraper(BaseScraper):
    """Handler cirúrgico para o InfoMoney."""

    name = "infomoney"
    domains = ("infomoney.com.br",)

    def wait_for_content(self, page: Any) -> bool:
        """Espera o artigo carregar no DOM."""
        for _ in range(15):
            try:
                info = page.evaluate(_WAIT_INFOMONEY_CONTENT_JS)
                if info.get("found"):
                    return True
            except Exception:
                pass
    ...
    def cleanup(self, page: Any) -> dict:
        """Remoção cirúrgica de anúncios e modais do InfoMoney."""
        try:
            result = page.evaluate(_CLEANUP_INFOMONEY_JS)
            return {"handler": "infomoney", **result}
        except Exception as exc:
            return {"handler": "infomoney", "error": str(exc)[:300]}
