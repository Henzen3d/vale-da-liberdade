#!/usr/bin/env python3
"""Scraper cirúrgico para o Blog do BG (blogdobg.com.br).

Arquitetura do site:
- CMS WordPress focado em notícias políticas, bastidores e cobertura regional/nacional.
- O texto integral da matéria vem no HTML original dentro de ``article``, ``.entry-content`` ou ``.post-content``.
- Sem paywall, com anúncios laterais e de topo.
- O cabeçalho com o logo do Blog do BG é mantido no topo em posição estática.
"""
...
# ---------------------------------------------------------------------------
# JS cirúrgico de limpeza para Blog do BG
# ---------------------------------------------------------------------------

_CLEANUP_BG_JS = """() => {
  const removed = [];

  // 1. Remover banners de publicidade e sidebars
  const adSelectors = [
    '[id*="google_ads"]',
    '[id*="ad_"]',
    '[class*="publicidade"]',
  ...
  // 3. Tornar o cabeçalho institucional (logo Blog do BG) estático e visível
...
@register("blogdobg.com.br")
class BlogDoBgScraper(BaseScraper):
    """Handler cirúrgico para o Blog do BG."""

    name = "blogdobg"
    domains = ("blogdobg.com.br",)

    def wait_for_content(self, page: Any) -> bool:
        """Espera o artigo carregar no DOM."""
        for _ in range(15):
            try:
                info = page.evaluate(_WAIT_BG_CONTENT_JS)
                if info.get("found"):
                    return True
            except Exception:
                pass
    ...
    def cleanup(self, page: Any) -> dict:
        """Remoção cirúrgica de anúncios do Blog do BG."""
        try:
            result = page.evaluate(_CLEANUP_BG_JS)
            return {"handler": "blogdobg", **result}
        except Exception as exc:
            return {"handler": "blogdobg", "error": str(exc)[:300]}
